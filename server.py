...
def build():
    ...
    if player_pos in undeveloped_levels:
        ...
        return grid
    choice = get_client_input("Enter the structure to build: log holder ('l'), house ('h'), bridge ('b'), campfire ('c'), garden ('g'),farm ('f'), quarry ('q'), metalworks ('m'), river ('r') or enter to close:")
    pos = get_client_input("Enter the movement direction to build in: 'WASD'")
...
def destroy():
    ...
    global player_col
    global houses
    direction = get_client_input("What adjacent tile do you want to destroy? (WASD or ENTER to close):")
    coords = select_new(direction)
    new_row = coords[0]
    new_col = coords[1]
    if new_row == player_row and new_col == player_col:
...
def interact():
    ...
    global player_col
    global houses
    direction = get_client_input("What adjacent tile do you want to interact with? (WASD or ENTER to close):")
    coords = select_new(direction)
    new_row = coords[0]
    new_col = coords[1]
    if new_row == player_row and new_col == player_col:
    ...
    if grid[new_row][new_col].isnumeric():
        ...
        choice = get_client_input("Place logs ('p') or take logs ('t')?:")
        number = get_client_input("How many?:")
        if number.isnumeric():
            number = int(number)
        else:
            number = 0
    ...
    if grid[new_row][new_col] in path_levels:
        send_data("You can use level 3 and above paths to move materials mined by workers.")
        if path_levels.index(grid[new_row][new_col])>=2:
            while True:
                selection = get_client_input("Do you want to take materials ('t'), place materials ('p'), or press enter to exit:")
                if selection == "t":
                    choice  = get_client_input("Do you want to take logs ('l'), stone ('s'), metal ('m'), or press enter to return:")
                    number = get_client_input("How many do you want to take?:")
                    if number.isnumeric():
                        number = int(number)
                    else:
                        number = 0
                ...
                if selection == "p":
                    choice  = get_client_input("Do you want to place logs ('l'), stone ('s'), metal ('m'), or press enter to return:")
                    number = get_client_input("How many do you want to place?:")
                    if number.isnumeric():
                        number = int(number)
                    else:
                        number = 0
...
def upgrade():
    ...
    global grid
    global houses
    direction = get_client_input("What adjacent tile do you want to upgrade? (WASD or ENTER to close):")
    coords = select_new(direction)
    new_row = coords[0]
    new_col = coords[1]
...
def assign_people():
    while True:
        choice = get_client_input("To assign to a category, enter ('a').  To remove from a category, enter ('r').  If you are satisfied, just press enter:")
        if choice == "a":
            category = get_client_input("Assign them to: logging ('l'), quarrying ('q'), metalworking ('m'), forest improvements ('i'), or press enter to return:")
            number = get_client_input("How many people will you assign?:")
            if number.isnumeric():
                number = int(number)
            else:
                number = 0
            ...
            category = get_client_input("Remove from: logging ('l'), quarrying ('q'), metalworking ('m'), forest improvements ('i'), or press enter to return:")
            number = get_client_input("How many people will you remove?:")
            if number.isnumeric():
                number = int(number)
            else:
                number = 0
...
def handler(choice):
    ...
    if choice == "a" or choice == "w" or choice == "s" or choice == "d":
        ...
        get_client_input("Press enter when done:")
    elif choice == "c":
        save()
    return grid
...
new_game = get_client_input("Do you want to start a new game ('s') or load an old one? ('l'):")
if new_game == "l":
    while True:
        save_name = get_client_input("Enter the filename, without .txt:")
        try:
            load(save_name)
            break
        except:
            choice = get_client_input("This is not a valid save. Type 'n' to make a new save, or enter to try again.")
            if choice=="n":
                new_game = "s"
                break
if new_game!="l":
    save_name = get_client_input("What will the save name be?:")
    if save_name.strip()=="":
        save_name = str(date.today())
    send_data(help_string)
    choice = get_client_input("You are about to generate a random map based on some presets.  Do you want normal ('n'), thick forest ('f'), thin forest ('t'), islands ('i'), or swamp ('s')?:")
    spawns = 0
    spread = 0
    cycles = 0
    tolerance = 0
...
while True:
    bring_people() 
    choice = get_client_input("Make your move:")
    ...
    send_data(data)
"""
Editing send_dataed text probably near impossible in py
